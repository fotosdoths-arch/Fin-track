[index.html](https://github.com/user-attachments/files/30063834/index.html)
<!DOCTYPE html>
<html lang="pt-BR">
<head>
<meta charset="UTF-8">
...
<div id="lock-screen" class="lock-screen">
  <div class="lock-card">
    ...
    <div class="lock-sub">Digite a senha para acessar</div>
...
<script>
// ===================== LOCK SCREEN =====================
const PASS_HASH='contths123'; // stored as plaintext — obfuscated via check only
...
function openModal(id){
  ...
  document.getElementById('f-amount').oninput=()=>{if(document.getElementById('f-parceled').checked)calcParcelaValor()};
...
async function fetchFocusSelic(){
  try{
    const r=await fetch('https://api.bcb.gov.br/dados/serie/bcdata.sgs.13521/dados/ultimos/12?formato=json');
    const data=await r.json();
    if(data&&data.length){
      // pega a mais recente
      const val=parseFloat(data[data.length-1].valor);
      return val;
    }
  }catch{}
  return null;
...
async function fetchFocusIPCA(){
  try{
    const r=await fetch('https://api.bcb.gov.br/dados/serie/bcdata.sgs.13522/dados/ultimos/12?formato=json');
    const data=await r.json();
    if(data&&data.length){
      const val=parseFloat(data[data.length-1].valor);
      return val;
...
async function renderPatrimonio(){
  ...
  if(selicVal!=null){
    ...
    document.getElementById('pat-selic-hint').textContent=`Focus BCB: ${selicVal.toFixed(2)}% a.a.`;
    selicBadge.className='pat-api-badge pat-api-ok'; selicBadge.textContent=`✓ SELIC ${selicVal.toFixed(2)}%`;
  } else {
    selicBadge.className='pat-api-badge pat-api-err'; selicBadge.textContent='✗ Erro ao buscar SELIC';
    document.getElementById('pat-selic-hint').textContent='Insira manualmente';
  }

  if(ipcaVal!=null){
    const ipcaInput=document.getElementById('pat-ipca');
    if(!ipcaInput.value) ipcaInput.value=ipcaVal.toFixed(2);
    document.getElementById('pat-ipca-hint').textContent=`Focus BCB: ${ipcaVal.toFixed(2)}% a.a.`;
    ipcaBadge.className='pat-api-badge pat-api-ok'; ipcaBadge.textContent=`✓ IPCA ${ipcaVal.toFixed(2)}%`;
  } else {
    ipcaBadge.className='pat-api-badge pat-api-err'; ipcaBadge.textContent='✗ Erro ao buscar IPCA';
    document.getElementById('pat-ipca-hint').textContent='Insira manualmente';
  }
