[index.html](https://github.com/user-attachments/files/28408104/index.html)
<!DOCTYPE html>
<html lang="pt-BR">
<head>
<meta charset="UTF-8">
...
.login-screen{
  position:fixed;inset:0;background:linear-gradient(135deg,#0f1117,#171b26);
  display:flex;align-items:center;justify-content:center;z-index:9999;padding:20px
}
.login-card{
  width:100%;max-width:420px;background:var(--bg2);border:1px solid var(--border2);
...
.login-error{
  display:none;background:var(--red-bg);border:1px solid rgba(245,101,101,.3);
...
<div id="login-screen" class="login-screen">
  <div class="login-card">
    ...
    <div class="login-sub">Digite o código de acesso para visualizar os dados financeiros.</div>
    ...
    <div class="login-actions">
      <button class="btn btn-primary" style="width:100%" onclick="checkAccess()">
        Entrar
      </button>
    </div>
  </div>
...
<div id="app-content" class="hidden-app">
...
const ACCESS_CODE='contabilidadeths@123';
...
function checkAccess(){
  ...
  if(input.value===ACCESS_CODE){
    ...
    sessionStorage.setItem('fintrack_access','granted');
  }else{
...
function verifySession(){
  const granted=sessionStorage.getItem('fintrack_access');
  if(granted==='granted'){
    document.getElementById('login-screen').style.display='none';
    document.getElementById('app-content').classList.remove('hidden-app');
  }
